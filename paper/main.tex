...
Lemoine's conjecture \cite{lemoine}, independently restated by Levy
\cite{levy}, asserts:
...
conjecture, do not establish an exhaustive bound. Zhang \cite{zhang} gives an
equivalent form of a strong version of the conjecture. Two claimed
proofs \cite{agama, ghanim} have not gained acceptance --- the argument
of \cite{agama} was subsequently found to be flawed --- and the
conjecture remains open.

Our contributions are: (i) a single reproducible exhaustive verification
to $2\times10^{13}$, producing both minimal-prime record sequences, one
of which reproduces the full A002091 $b$-file and extends it by one term;
(ii) exact representation counts $r(n)$ for all odd $n\le10^7$ via FFT
convolution, extended by windowed counts to $10^8$; (iii) a numerical test of the
...
We model the primality of the two components as independent events: an
integer of size $t$ is prime with ``probability'' $1/\log t$. For odd
$n$ and a random $q \in [3, (n-3)/2]$, set $p = n - 2q$. The naive
expectation for $r(n)$ is then
...
We use a segmented, odd-only bitmap sieve. For each odd $n$ the program
searches the minimal $q$ (over a fixed list of primes $q < 2\times10^5$)
with $n-2q$ prime, using a primality window extending $4\times10^5$
below the current segment; a second window at half height serves the
mirrored decomposition $n = 2p+q$. Segments of length $2^{28}$ are
distributed over eight threads. Work is dominated by the segmented
sieving:
...
total; Table~\ref{tab:records} shows the tail), and the minimal undoubled prime attains
$q_{\min} = 9157$ at $n = 14\,480\,831\,322\,263$ (50 records). The
noncomposite variant of the undoubled-side record sequence (records of
A185091 \cite{oeis185091}) reproduces
...
(50 records, tail shown). Complete lists are in the repository.}
\label{tab:records}
...
records with $n > 10^3$ gives $\alpha_A = 0.596$ (39 records) for
$n = p+2q$ and $\alpha_B = 1.196$ (41 records) for $n = 2p+q$. The ratio
$\alpha_B/\alpha_A = 2.01$ has a clean explanation: in
...
The representation function $r(n)$ (OEIS A046927 \cite{oeis46927}) for
all odd $n \le 10^7$ is the convolution of the prime indicator
...
The computations, analysis, and drafting of this paper were carried out
in collaboration with Claude (Anthropic), an AI assistant, under the
direction of the author, who takes responsibility for the content. The
algorithms, code, and statistical analyses were substantially produced
by the AI system; all computational results are independently
reproducible from the open-source repository below.

\section{Data and code availability}
All programs (C++ verifier, Python analysis scripts), the complete
record sequences, $q_{\min}$ distribution histograms, fit data, and the
scripts generating every figure are available at
\url{https://github.com/YunyueLi/lemoine-conjecture}.
...
\bibitem{mathworld-levy} E. W. Weisstein, Levy's Conjecture, MathWorld, \url{https://mathworld.wolfram.com/LevysConjecture.html}.
\bibitem{brainhappy} MakeTheBrainHappy, Lemoine's conjecture verified to $10^{10}$, blog post, June 2019, \url{https://www.makethebrainhappy.com/2019/06/lemoines-conjecture-verified-to-1010.html}.
...
\bibitem{agama} T. Agama and B. Gensel, A proof of Lemoine's conjecture by circles of partition, arXiv:1709.05335; the argument was subsequently found to be flawed.
